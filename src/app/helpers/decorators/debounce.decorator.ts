...
export function DebounceMethod(target: Object, propertyName: string, descriptor: PropertyDescriptor) {
  const originalMethod = descriptor.value;

  descriptor.value = function (...args: any) {
    if (Reflect.get(target, 'subscription')) return;
    Reflect.set(target, 'subscription', fromEvent(window, 'resize').pipe(
