...
export class PostService {
  ...
  fetchPosts(userId: number): Observable<Post[]> {
    return this.#http.get<Post[]>(`${baseUrl}post/?user_id=${userId}`)
      .pipe(
        tap(posts => {
  ...
  createPost(payload: PostCreateDto): Observable<Post[]> {
    return this.#http.post<Post>(`${baseUrl}post/`, payload)
      .pipe(
        switchMap(() => {
  ...
  createComment(payload: CommentCreateDto) {
    return this.#http.post<PostComment>(`${baseUrl}comment/`, payload)
      .pipe(
        switchMap(() => this.fetchPosts(payload.authorId))
      );
  }
  ...
  getCommentsByPostId(postId: number) {
    return this.#http.get<Post>(`${baseUrl}post/${postId}`)
      .pipe(
        map(post => this.#sortById(post.comments, true))
      );
  }
